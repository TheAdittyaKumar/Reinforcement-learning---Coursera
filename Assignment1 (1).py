...
def argmax(q_values):
    ...
    """
    top_value = float("-inf") 
    #best value seen so far (start below any real number)
    ties = []
    #indices currently tied for the best value
    
    #Scan through q_values by index so we can return the index of the winner
    for i in range(len(q_values)):
        # Found a NEW strictly larger maximum:
        # 1) update the best value
        if q_values[i]>top_value:
            top_value=q_values[i]
            # 2) reset ties to only this index (old ties are no longer max)
            ties=[i]
        # if a value in q_values is greater than the highest value update top and reset ties to zero
        elif q_values[i]==top_value:
            # Found another element equal to the current maximum -> keep as a tie
            ties.append(i)
            # If v < top_value, do nothing (it isn't a contender)
...
class GreedyAgent(main_agent.Agent):
    def agent_step(self, reward, observation=None):
        ...
        # your code here
        a=self.last_action
        # which arm did we pull last step?
        if a is not None: # skip if this is the very first step
            self.arm_count[a]+=1  # we just pulled arm a one more time
            alpha=1/self.arm_count[a] # step size = 1 / pulls of a
            # move Q[a] a bit toward the reward we just saw
            self.q_values[a]+=alpha*(reward-self.q_values[a])
        # pick the arm with the largest Q (break ties at random)
        ...
        # remember what we chose so we can update it next call
...
class EpsilonGreedyAgent(main_agent.Agent):
    def agent_step(self, reward, observation):
        ...
        # your code here
         # which arm did we pull previously?
        a=self.last_action
        if a is not None: # skip if this is the very first call
            self.arm_count[a]+=1 # we have pulled arm a one more time
            alpha=1/self.arm_count[a] # step size = 1 / pulls of a
            # move Q[a] toward the reward we just saw
            self.q_values[a]+=alpha*(reward-self.q_values[a])
        # ε-greedy choice of the NEXT action    
        if np.random.random()<self.epsilon:
            # explore: pick any arm uniformly at random
            current_action=int(np.random.randint(self.num_actions))
        else: # exploit: pick the arm with the largest Q (ties broken randomly)
            current_action=int(argmax(self.q_values))
...
class EpsilonGreedyAgentConstantStepsize(main_agent.Agent):
    def agent_step(self, reward, observation):
        ...
        a=self.last_action # previous arm index
        if a is not None: # skip if first call (no previous arm)
            self.arm_count[a]+=1 # book-keeping; not used for alpha here
            alpha=float(self.step_size) # CONSTANT step size α
            # move Q[a] toward the new reward by a fraction α
            self.q_values[a]+=alpha*(reward-self.q_values[a])
        # --- choose the next action using ε-greedy ---
        if np.random.random()<self.epsilon: # explore: uniform random arm
            current_action=int(np.random.randint(self.num_actions))
        else: # exploit: arm with largest Q (ties broken randomly by argmax)
            current_action=int(argmax(self.q_values))
